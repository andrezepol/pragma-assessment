    ...
    try {
        ...
        if (data.state === 'Vendido') {
            return {
                ...
                body: JSON.stringify({ message: "Unidad no se encuentra disponible" }),
            };
        }

        if (data.state === 'Reservado' && sell.reserveToken !== data.reserveToken) {
            return {
                statusCode: 400,
                headers: {
                    "Content-Type": "application/json",
                ...
                body: JSON.stringify({ message: "Los id de reserva no son iguales" }),
            };
        }

        // Si llega aquí, significa que el estado no es 'Vendido' o es 'Reservado' y los id de reserva coinciden
        const newSell = {
            ...sell,
            buyId: code,
        };

        // Inserta el nuevo registro en DynamoDB
